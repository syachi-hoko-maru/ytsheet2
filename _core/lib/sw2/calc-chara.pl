...
sub data_calc {
  ...
  if($pc{lvSeeker}){
    foreach (1..5) {
      ...
      $pc{buildupAddAppar     }++ if $pc{'seekerBuildup'.$_} eq '妖術';
  ...
  ## 選択特技による補正
  {
    foreach my $i (@set::feats_lv) {
      ...
      elsif($feat eq '妖術追加Ⅰ')  { $pc{apparAddition} = 1; }
      elsif($feat eq '妖術追加Ⅱ')  { $pc{apparAddition} = 2; }
      elsif($feat eq '妖術追加Ⅲ')  { $pc{apparAddition} = 3; }
